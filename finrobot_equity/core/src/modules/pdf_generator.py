...
# =============================================================================
# 中文字体注册 (CID 方式，支持中英数混排)
# =============================================================================
_CJK_FONT_REGISTERED = False
try:
    pdfmetrics.registerFont(UnicodeCIDFont('STSong-Light'))
    _CJK_FONT_REGISTERED = True
    print(f"  📝 注册中文字体: STSong-Light (CID)")
except Exception as e:
    print(f"  ⚠️ CID 字体注册失败: {e}")
...
class FontSpec:
    """字体规格"""
    FONT_NORMAL = 'STSong-Light' if _CJK_FONT_REGISTERED else 'Helvetica'
    FONT_BOLD = 'STSong-Light' if _CJK_FONT_REGISTERED else 'Helvetica-Bold'
...
class EquityReportPDF:
    ...
    def __init__(self, output_path: str, report_data: Dict[str, Any]):
        ...
        # 自动检测语言：如果 tagline 或 company_overview 包含中文则用中文标题
        _text_sample = str(report_data.get('tagline', '')) + str(report_data.get('company_overview', ''))
        import re
        self.is_zh = bool(re.search(r'[\u4e00-\u9fff]', _text_sample))
        ...
        if self.is_zh:
            self.L = {
                ...
                'Free Float': '自由流通',
                'Dividend Yield': '股息率',
                'Net Debt/Equity': '净负债率',
                'Fwd PE': '远期 P/E',
                'PB': 'P/B',
                'ROE': 'ROE',
                'Ticker': '股票代码',
                'not_available': '暂无数据',
                'disclaimer': '免责声明：本报告由 FinRobot 自动生成，仅供参考，不构成投资建议。',
            }
        else:
            self.L = {
                'KEY_DATA': 'KEY DATA',
                ...
                'Rating': 'Rating',
                'Target Price': '12-mth Target Price',
                ...
                'disclaimer': 'Disclaimer: This report is auto-generated by FinRobot for reference only. Not investment advice.',
            }
        self.elements = []
    ...
    def add_key_data_table(self) -> Table:
        """创建KEY DATA表格"""
        L = self.L
        suffix = '' if self.is_zh else ' US'
        data = [
            [L['Ticker'], f"{self.data.get('company_ticker', 'N/A')}{suffix}"],
            [L['Sector'], self.data.get('sector', 'N/A')],
            [L['Share Price'], self.data.get('share_price', 'N/A')],
            [L['Rating'], self.data.get('rating', 'N/A')],
            [L['Target Price'], self.data.get('target_price', 'N/A')],
            [L['Market Cap'], self.data.get('market_cap', 'N/A')],
            [L['Volume'], self.data.get('volume', 'N/A')],
            [L['Free Float'], self.data.get('free_float', 'N/A')],
            [L['Dividend Yield'], self.data.get('dividend_yield', 'N/A')],
            [L['Net Debt/Equity'], self.data.get('net_debt_to_equity', 'N/A')],
            [L['Fwd PE'], self.data.get('fwd_pe', 'N/A')],
            [L['PB'], self.data.get('pb_ratio', 'N/A')],
            [L['ROE'], self.data.get('roe', 'N/A')],
        ]
    ...
    def add_page1_content(self):
        ...
        # KEY DATA 标题和表格
        self.elements.append(Paragraph(self.L['KEY_DATA'], self.styles['H2']))
        ...
        # Company Overview
        self.elements.append(Paragraph(self.L['COMPANY_OVERVIEW'], self.styles['H2']))
        overview = self.data.get('company_overview', self.L['not_available'])
        # 截断过长的内容
        ...
        # Investment Overview
        self.elements.append(Paragraph(self.L['INVESTMENT_OVERVIEW'], self.styles['H2']))
        investment = self.data.get('investment_overview', self.L['not_available'])
        if len(investment) > 1000:
            investment = investment[:1000] + "..."
        self.elements.append(Paragraph(investment, self.styles['Body']))
        
        # Valuation Analysis
        self.elements.append(Paragraph(self.L['VALUATION_ANALYSIS'], self.styles['H2']))
        valuation = self.data.get('valuation_overview', self.L['not_available'])
    ...
    def add_page1_bottom(self):
        ...
        # Risks
        self.elements.append(Paragraph(self.L['RISK_FACTORS'], self.styles['H2']))
        risks_text = self.data.get('risks', self.L['not_available'])
        # 将风险文本分割成列表项
        if risks_text:
    ...
    def add_page2_content(self):
        ...
        # 竞争分析
        self.elements.append(Paragraph(self.L['COMPETITIVE_ANALYSIS'], self.styles['H2']))
        competitor = self.data.get('competitor_analysis', self.L['not_available'])
        ...
        # 主要要点
        self.elements.append(Paragraph(self.L['KEY_POINTS'], self.styles['H2']))
